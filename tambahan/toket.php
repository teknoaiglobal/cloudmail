...
if ($action) {
    ...
    // API: Dapatkan isi spesifik dari satu file editor
    if ($action === 'get_file') {
        $filename = $_GET['filename'] ?? '';
        $files = json_decode(file_get_contents($files_file), true) ?: [];
        
        if ($filename && isset($files[$filename])) {
            echo json_encode([$filename => $files[$filename]]);
        } else {
                                        ...
                                        <div key={file} className="bg-slate-800 p-3 rounded-lg border border-slate-700">
                                            <div className="flex justify-between items-center mb-2">
                                                ...
                                                <div className="flex gap-2">
                                                    ...
                                                    <button onClick={() => copyToClipboard(`${baseUrl}?action=get_file&filename=${file}`)} className="text-[10px] bg-slate-700 hover:bg-slate-600 px-2 py-1 rounded text-slate-300 flex items-center gap-1">
